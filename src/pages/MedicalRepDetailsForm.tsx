...
type MedicalRepFormData = z.infer<typeof medicalRepSchema>;

const MedicalRepDetailsForm = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<MedicalRepFormData>({
    resolver: zodResolver(medicalRepSchema),
  });
  ...
  const onSubmit = async (data: MedicalRepFormData) => {
    if (!user) return;
    ...
    try {
      ...
      const { error } = await supabase.from('medical_rep_profiles').insert([{
        user_id: user.id,
      ...
      console.error('Error creating medical rep profile:', error);
...
export default MedicalRepDetailsForm;
